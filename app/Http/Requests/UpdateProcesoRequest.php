...
class UpdateProcesoRequest extends FormRequest
{
    ...
    {
        $procesoId = $this->route('proceso') ? $this->route('proceso')->id : null;
        ...
        return [
            ...
            'archivos_a_eliminar.*' => 'integer|exists:archivos,id',
            function ($attribute, $value, $fail) use ($procesoId) {
                $archivo = \App\Models\Archivo::find($value);
                if (
                    $archivo &&
                    ($archivo->archivable_type !== \App\Models\Proceso::class ||
                        $archivo->archivable_id !== $procesoId)
                ) {
                    $fail('El archivo no pertenece a este proceso.');
                }
            }
