...
<template>
    <LayoutMain>
        <SectionTitleLineWithButton :title="props.titulo" main :icon="mdiApps">
            <!-- <BaseButton :href="route('certificacion.form')" color="danger" label="Cargar Datos" /> --> 
            <!-- <BaseButton :href="route('certificacion.export', { search: filters.search })" color="success"
                label="Exportar excel" /> -->
            <BaseButton :href="route(`${props.routeName}create`)" color="warning" label="Registrar Nueva Certificación" :icon="mdiPlus" />
        ...
        <CardBox v-if="certificaciones.data.length < 1">
        ...
        <CardBox v-else class="mb-6" has-table>
            <table class="w-full border-collapse border mt-4">
                <thead>
                    <tr>
                        <th />
                        <th class="border p-2">Nombre de la Certificación</th>
                ...
                <tbody>
                    <tr v-for="documento in certificaciones.data" :key="documento.id">
                        ...
                        <td data-label="Estatus del sistema " class="border p-2">{{ documento.fecha_solicitud }} </td>
                        <td data-label="Estatus del sistema " class="border p-2">{{ documento.fecha_inicio_vigencia }} </td>
                        <td data-label="Estatus del sistema " class="border p-2">{{ documento.fecha_fin_vigencia }} </td>
            ...
            <Pagination :currentPage="certificaciones.current_page" :links="certificaciones.links" :total="certificaciones.last_page" />
        </CardBox>
    </LayoutMain>
</template>